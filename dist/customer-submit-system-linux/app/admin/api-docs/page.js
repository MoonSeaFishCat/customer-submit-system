        ...
        <Card className="glass-card">
          ...
          <CardContent className="grid gap-4 lg:grid-cols-2">
            <div className="rounded-md border p-4">
              ...
              <CodeBlock>{`x-api-key: 你的 API_SECRET`}</CodeBlock>
            </div>
            <div className="rounded-md border p-4">
              <h3 className="font-semibold">密钥在哪里配置？</h3>
              <p className="mt-2 text-sm text-muted-foreground">在项目的 .env 文件里配置：</p>
              <CodeBlock>{`ADMIN_SECRET=后台登录密码
API_SECRET=给接口调用用的密钥`}</CodeBlock>
              <p className="mt-2 text-sm text-muted-foreground">如果没配置 API_SECRET，系统会默认使用 ADMIN_SECRET。</p>
            </div>
          </CardContent>
        </Card>
