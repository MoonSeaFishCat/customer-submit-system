...
export async function createTemplate(input) {
  ...
  await query(
    `INSERT INTO templates (id, slug, name, description, fields, webhook_urls, webhook_headers, active, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      id,
      slug,
...
export async function updateTemplate(id, input) {
  ...
  await query(
    `UPDATE templates
     SET slug = ?, name = ?, description = ?, fields = ?, webhook_urls = ?, webhook_headers = ?, active = ?, updated_at = ?
     WHERE id = ?`,
    [
      next.slug,
      next.name,
...
export async function createSubmission({ template, data, source, ip, userAgent }) {
  ...
  const ts = nowIso();
  await query(
    `INSERT INTO submissions (id, template_id, template_slug, data, status, source, ip, user_agent, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      id,
      template.id,
...
export async function getSubmissions({ templateSlug, limit = 100 } = {}) {
  const sql = templateSlug
    ? "SELECT * FROM submissions WHERE template_slug = ? ORDER BY created_at DESC LIMIT ?"
    : "SELECT * FROM submissions ORDER BY created_at DESC LIMIT ?";
  const params = templateSlug ? [templateSlug, Number(limit)] : [Number(limit)];
