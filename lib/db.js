...
function normalizeRows(rows) {
  return rows.map((row) => ({
    ...
    erp_order_data: safeJsonParse(row.erp_order_data, null),
    erp_anomaly_override: row.erp_anomaly_override || null,
  }));
}
...
async function ensureSubmissionPushColumns() {
  if (DB_CLIENT === "mysql") {
    ...
    try {
      await query("ALTER TABLE submissions ADD COLUMN erp_order_data JSON");
    } catch (error) {
      if (!String(error.message || "").toLowerCase().includes("duplicate")) {
        throw error;
      }
    ...
    try {
      await query("ALTER TABLE submissions ADD COLUMN erp_anomaly_override VARCHAR(40)");
    } catch (error) {
      if (!String(error.message || "").toLowerCase().includes("duplicate")) {
        throw error;
      }
  ...
  if (!columnNames.has("erp_order_data")) {
    await query("ALTER TABLE submissions ADD COLUMN erp_order_data TEXT");
  }

  if (!columnNames.has("erp_anomaly_override")) {
    await query("ALTER TABLE submissions ADD COLUMN erp_anomaly_override TEXT");
  }
...
export async function getSubmissions({ templateSlug, limit = 100, page, pageSize, search, startDate, endDate, status, source, pushStatus, erpStatus, ip, fieldFilters = [] } = {}) {
  const conditions = [];
  const params = [];
  ...
  if (pushStatus) {
    ...
    params.push(pushStatus);
  }
  if (erpStatus) {
    if (erpStatus === "has_order") {
      conditions.push("erp_order_data IS NOT NULL AND erp_order_data != ''");
    } else if (erpStatus === "no_order") {
      conditions.push("(erp_order_data IS NULL OR erp_order_data = '')");
    } else if (erpStatus === "anomaly") {
      conditions.push("erp_order_data LIKE ?");
      params.push('%"anomaly":true%');
    } else if (erpStatus === "normal") {
      conditions.push("erp_order_data IS NOT NULL AND erp_order_data != '' AND erp_order_data NOT LIKE ?");
      params.push('%"anomaly":true%');
    }
  }
  if (ip) {
  ...
  // 字段筛选
  if (fieldFilters && Array.isArray(fieldFilters) && fieldFilters.length > 0) {
    for (const filter of fieldFilters) {
      ...
      switch (operator) {
        ...
        case "not_equals":
          if (DB_CLIENT === "mysql") {
            conditions.push("(JSON_EXTRACT(data, ?) != ? OR JSON_EXTRACT(data, ?) IS NULL)");
            params.push(jsonPath, value, jsonPath);
          } else {
            conditions.push("(json_extract(data, ?) != ? OR json_extract(data, ?) IS NULL)");
            params.push(jsonPath, value, jsonPath);
          }
        ...
        case "contains":
          ...
          break;
        case "not_contains":
          conditions.push("(data NOT LIKE ? OR data NOT LIKE ?)");
          params.push(`%"${field}"%`, `%${value}%`);
          break;
        case "starts_with":
          conditions.push("data LIKE ?");
        ...
        case "gt":
          if (DB_CLIENT === "mysql") {
            conditions.push("CAST(JSON_EXTRACT(data, ?) AS DECIMAL) > ?");
            params.push(jsonPath, Number(value));
          } else {
            conditions.push("CAST(json_extract(data, ?) AS REAL) > ?");
            params.push(jsonPath, Number(value));
          }
          break;
        case "gte":
        ...
        case "empty":
          if (DB_CLIENT === "mysql") {
            conditions.push("(JSON_EXTRACT(data, ?) IS NULL OR JSON_EXTRACT(data, ?) = '' OR JSON_EXTRACT(data, ?) = '\"\"')");
        ...
        case "not_empty":
          if (DB_CLIENT === "mysql") {
            conditions.push("(JSON_EXTRACT(data, ?) IS NOT NULL AND JSON_EXTRACT(data, ?) != '' AND JSON_EXTRACT(data, ?) != '\"\"')");
            params.push(jsonPath, jsonPath, jsonPath);
          } else {
            conditions.push("(json_extract(data, ?) IS NOT NULL AND json_extract(data, ?) != '' AND json_extract(data, ?) != '\"\"')");
            params.push(jsonPath, jsonPath, jsonPath);
...
export async function updateSubmissionErpOrder(id, erpOrderData) {
  const current = await getSubmissionById(id);
  if (!current) return null;
  await query(
    "UPDATE submissions SET erp_order_data = ?, updated_at = ? WHERE id = ?",
    [jsonStringify(erpOrderData), nowIso(), id]
  );
  return getSubmissionById(id);
}

export async function updateSubmissionErpOverride(id, override) {
  const current = await getSubmissionById(id);
  if (!current) return null;
  await query(
    "UPDATE submissions SET erp_anomaly_override = ?, updated_at = ? WHERE id = ?",
    [override || null, nowIso(), id]
  );
  return getSubmissionById(id);
}
