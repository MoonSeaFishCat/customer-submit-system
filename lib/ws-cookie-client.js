...
function writeCache(cache) {
  try {
    fs.mkdirSync(require("node:path").dirname(CACHE_FILE), { recursive: true });
 ...
 * @returns {object|null}
 */
export function getCachedCookie(platform, store, account) {
  return readCache()[cacheKey(platform, store, account)] ?? null;
}
 ...
 * @returns {object|null}
 */
export function popCachedCookie(platform, store, account) {
  const cache = readCache();
