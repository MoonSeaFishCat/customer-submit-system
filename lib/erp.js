...
import { getCachedErpOrders, setCachedErpOrders } from "@/lib/erp-cache";
...
async function getErpCookie() {
  ...
  // 先用缓存
  const cached = await getCachedCookie(ERP_PLATFORM, ERP_STORE, ERP_ACCOUNT);
...
export async function searchErpOrders(query) {
  ...
  // 检查缓存
  const cached = getCachedErpOrders(query);
  if (cached) {
    console.log("[ERP] 使用缓存结果, query=", query);
    return cached;
  }
  ...
  console.log("[ERP] byNick raw: result=", byNick?.result, "msg=", byNick?.msg ?? byNick?.message, "total=", byNick?.data?.total, "listLen=", byNick?.data?.list?.length ?? 0);
  const list = byNick?.data?.list;
  if (Array.isArray(list) && list.length > 0) {
    const mapping = byNick.data.fieldMapping || FIELD_MAP;
    const result = list.map((o) => normalizeOrder(o, mapping));
    setCachedErpOrders(query, result);
    return result;
  }

  // 无结果则按订单号查
  const byTid = await doSearch(query, true);
  console.log("[ERP] byTid raw: result=", byTid?.result, "msg=", byTid?.msg ?? byTid?.message, "total=", byTid?.data?.total, "listLen=", byTid?.data?.list?.length ?? 0);
  const list2 = byTid?.data?.list;
  if (Array.isArray(list2) && list2.length > 0) {
    const mapping = byTid.data.fieldMapping || FIELD_MAP;
    const result = list2.map((o) => normalizeOrder(o, mapping));
    setCachedErpOrders(query, result);
    return result;
  }
...
// 安装费补拍商品 ID（数量是金额份数，只算1台）
const INSTALL_REPAY_IDS = new Set([
...
export function analyzeInstallCount(orders, submissionCreatedAt, submittedQty) {
  ...
  for (const order of recentOrders) {
    ...
    for (const o of subOrders) {
      const ids = [String(o.numIid || ""), String(o.outerIid || ""), String(o.skuId || "")];
      const isInstall = ids.some((id) => INSTALL_ITEM_IDS.has(id));
      const isRepay = ids.some((id) => INSTALL_REPAY_IDS.has(id));
      const isInstallByTitle = String(o.sysTitle || o.title || "").includes("AZ-上门安装服务");

      if (isInstall || isInstallByTitle) {
        // 真实安装服务：累加数量（3台设备买了3个安装服务）
        orderInstallQty += Number(o.num) || 0;
      } else if (isRepay) {
        // 安装费补拍：只标记有，整笔订单算1台（金额补拍数量无意义）
        hasRepay = true;
      }
    }
    // 补拍且没有真实安装商品时，算1台
    if (orderInstallQty === 0 && hasRepay) {
      orderInstallQty = 1;
    }
    // 卖家备注含"安装费"且订单本身没有安装商品时，按1台计入
    const sellerMemo = String(order.sellerMemo || "");
    if (orderInstallQty === 0 && sellerMemo.includes("安装费")) {
      orderInstallQty = 1;
    }
