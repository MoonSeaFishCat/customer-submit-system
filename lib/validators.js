...
export const fieldSchema = z.object({
  ...
  type: z.enum(["text", "tel", "email", "number", "textarea", "select", "date", "checkbox"]).default("text"),
...
export function validateSubmissionData(template, data) {
  const errors = {};
  const normalized = {};
  ...
  for (const field of template.fields || []) {
    ...
    normalized[field.key] = field.type === "checkbox" ? Boolean(value) : String(value).trim();
  }
